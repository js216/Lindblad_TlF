class CoupledBasisState:
    # constructor
    def __init__(self, F, mF, F1, J, I1, I2, Omega = 0, P = None, electronic_state = None, energy = None):
        self.F, self.mF  = F, mF
        self.F1 = F1
        self.J = J
        self.I1 = I1
    ...
    #Method for transforming Omega eigenstate to parity eigenstate basis
    def transform_to_parity_basis(self):
        ...
        if P == None and not electronic_state == 'X':
            state_minus = 1*CoupledBasisState(F,mF,F1,J,I1,I2,Omega = np.abs(Omega), P = -1, electronic_state=electronic_state)
            state_plus = 1*CoupledBasisState(F,mF,F1,J,I1,I2,Omega = np.abs(Omega), P = +1, electronic_state=electronic_state)
            
            if np.sign(Omega) < 0:
                state = (-1)**(J+1)/np.sqrt(2)  * (state_plus - state_minus)
            elif np.sign(Omega) > 0:
                state = 1/np.sqrt(2)  * (state_plus + state_minus)
        else:
            state = 1*self
...
class UncoupledBasisState:
    # constructor
    def __init__(self, J, mJ, I1, m1, I2, m2, Omega = 0, P = None, electronic_state = None, energy = None):
        self.J, self.mJ  = J, mJ
    ...
    def transform_to_parity_basis(self):
        ...
        #Check that not already in omega basis
        if not P == None and not electronic_state == 'X':
            state_minus = 1*UncoupledBasisState(J, mJ, I1, m1, I2, m2, P = -1, Omega=np.abs(Omega), electronic_state = electronic_state)
            state_plus = 1*UncoupledBasisState(J, mJ, I1, m1, I2, m2, P = +1, Omega=np.abs(Omega), electronic_state = electronic_state)
